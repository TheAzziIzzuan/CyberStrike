...
// --- SSRF DETECTION IN BACKGROUND ---
// --- SSRF Detection Using Declarative Net Request ---
...
// Monitor network activity and log SSRF attempts
chrome.webRequest.onBeforeRequest.addListener(
    (details) => {
        const sensitivePaths = ["/api/public-data", "/admin", "/restricted", "/internal", "/server-status", "/debug", "/config"];

        if (sensitivePaths.some(path => details.url.includes(path))) {
            console.warn(`🚨 SSRF Attempt Detected! Request to: ${details.url}`);

            chrome.runtime.sendMessage({
                type: "ssrf_detected",
                url: details.url,
                timestamp: new Date().toLocaleString(),
                requestData: "Detected via background.js"
            });
        }
    },
    { urls: ["<all_urls>"] },
