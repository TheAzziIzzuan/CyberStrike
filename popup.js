...
document.getElementById("scanSSRFButton").addEventListener("click", async () => {
    ...
    const baseEndpoints = [
        "/admin", "/auth", "/user", "/config", "/secrets",
        "/debug", "/internal", "/private", "/hidden",
        "/server-status", "/logs", "/health", "/status",
        "/api", "/api/v1", "/api/v2", "/payments",
        "/storage", "/s3", "/aws", "/metadata",
        "/keys", "/env", "/test", "/staging"
    ];
    const extensions = ["", ".php", ".json", ".xml", ".yml", ".ini"];
    const potentialAPIs = baseEndpoints.flatMap(endpoint => extensions.map(ext => `${endpoint}${ext}`));

    const ssrfTargets = [
        // Localhost & Internal Networks
        ...
        // Internal Corporate Services & DevOps APIs
        "http://intranet.company.com", "http://internal.company.com",
        "http://kubernetes.default.svc/", "http://10.0.0.1:6443",
        "http://172.17.0.1", "http://127.0.0.1:2375", 

        // Database Services
        "http://127.0.0.1:3306", "http://127.0.0.1:5432",
        "http://127.0.0.1:9200/_cat/indices", "http://127.0.0.1:5984/_all_dbs",

        // Miscellaneous Exploitable Services
        "http://127.0.0.1:8888", "http://127.0.0.1:3000",
        "http://127.0.0.1:5000", "http://127.0.0.1:8081"
    ];
    ...
    try {
        ...
        chrome.scripting.executeScript({
            target: { tabId: tab.id },
            func: async (apiUrls, ssrfUrls) => {
                let results = [];

                const apiRequests = apiUrls.map(async path => {
                    try {
                        let response = await fetch(window.location.origin + path, { method: "GET" });
                        if (response.ok) results.push(`🚨 Open API detected: ${path}`);
                    } catch (_) { /* Suppressed */ }
                });

                const ssrfRequests = ssrfUrls.map(async url => {
                    try {
                        let response = await fetch(url, { method: "GET", mode: "no-cors", cache: "no-store" });
                        if (response) results.push(`🚨 Possible SSRF detected: ${url}`);
                    } catch (_) { /* Suppressed */ }
                });

                await Promise.all([...apiRequests, ...ssrfRequests]); 
                return results;
            },
            args: [potentialAPIs, ssrfTargets]
        }, (injectionResults) => {
            if (injectionResults && injectionResults[0].result.length > 0) {
                outputDiv.innerHTML = injectionResults[0].result.join("<br>");
            } else {
                outputDiv.innerHTML = "✅ No SSRF risks detected.";
            }
        });
    } catch (error) {
